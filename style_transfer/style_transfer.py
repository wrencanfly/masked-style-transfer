...
class ContentLoss(nn.Module):
    ...
    def forward(self, input):
        return self.loss(input * self.mask, self.target * self.mask)
...
class SmoothnessLoss(nn.Module):
    def __init__(self, mask, weight=1.0):
        super(SmoothnessLoss, self).__init__()
        self.mask = mask
        self.weight = weight

    def forward(self, input):
        # Calculate edges in the mask
        mask_edge_x = torch.abs(self.mask[:, :, 1:] - self.mask[:, :, :-1])
        mask_edge_y = torch.abs(self.mask[:, 1:, :] - self.mask[:, :-1, :])

        # Calculate input image gradients
        input_edge_x = input[:, :, 1:] - input[:, :, :-1]
        input_edge_y = input[:, 1:, :] - input[:, :-1, :]

        # Multiply gradients by mask edges to focus only on transition areas
        edge_gradient_x = input_edge_x * mask_edge_x
        edge_gradient_y = input_edge_y * mask_edge_y

        # Calculate smoothness loss
        smooth_loss_x = edge_gradient_x.pow(2).mean()
        smooth_loss_y = edge_gradient_y.pow(2).mean()

        # Return weighted smoothness loss
        return self.weight * (smooth_loss_x + smooth_loss_y)
    

class BoundarySmoothLoss(nn.Module):
    def __init__(self, mask, weight=10):
        super().__init__()
        self.register_buffer('mask', mask)
        self.weight = weight

    def forward(self, input):
        dx, dy = torch.diff(input * self.mask, dim=2), torch.diff(input * self.mask, dim=3)
        return self.weight * (dx.abs().mean() + dy.abs().mean())
...
class StyleTransfer:
    ...
    def resize_mask(mask, target_size):
        return F.interpolate(mask.unsqueeze(0), size=target_size, mode='nearest').squeeze(0)
    ...
    def stylize(self, content_image, style_images, mask, *,
        ...
        if init == 'content':
            ...
            self.mask = TF.to_tensor(mask.resize((cw, ch), Image.BICUBIC))[None]
        elif init == 'gray':
